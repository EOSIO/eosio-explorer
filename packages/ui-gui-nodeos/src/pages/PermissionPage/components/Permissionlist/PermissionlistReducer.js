...
const actionPrefix = `PermissionPage/Permissionlist/`;
  ...
  mergeMap(action =>
    interval(500).pipe(
      mergeMap(action => {
          ...
          return from(new Promise((resolve, reject)=>{ resolve({response:[{account:"eosio"},{account:"terry"},{account:"chris"}]})})).pipe(
